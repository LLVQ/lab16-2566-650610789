...
export const GET = async (request) => {
  ...
  if(studentId !== null){
    filtered = filtered.filter((x) => x.studentId === studentId);
  }
...
export const DELETE = async (request) => {
  ...
  if(parseResult.success === false){
    return NextResponse.json({ok: false, message: parseResult.error.issues[0].message},{status: 400});

  }

  const foundIndex = DB.students.findIndex((x) => x.studentId === body.studentId);

  if(foundIndex === -1){
    return NextResponse.json({ok: false, message: "Student Id does not exist"},{status: 404});
